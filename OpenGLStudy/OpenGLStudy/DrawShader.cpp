...
void ShaderWithColorVAO()
{
	...
	//将创建的顶点着色器和顶点着色器代码字符串绑定
	std::string vertex_shader_string = ReadEntireFile("ShaderFile/VSNormal.txt");
	std::cout << vertex_shader_string << std::endl;
	const char* shader_string = vertex_shader_string.c_str();
	glShaderSource(vertexShader, 1, &shader_string, NULL);
	glCompileShader(vertexShader);
	...
	//将创建的片段着色器和片段着色器代码绑定
	std::string fragment_shader_string = ReadEntireFile("ShaderFile/FSNormal.txt");
	std::cout << fragment_shader_string << std::endl;
	shader_string = fragment_shader_string.c_str();
	glShaderSource(fragmentShader, 1, &shader_string, NULL);
	glCompileShader(fragmentShader);
...
void ShaderWithShaderObject()
{
    ...
    Shader ourShader("ShaderFile/VSWithShaderClass.vs", "ShaderFile/FSWithShaderClass.fs");
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        ourShader.use();
        ourShader.setFloat("someUniform", redValue);
...
void ShaderDraw()
{
    //ShaderWithVSAndFSFile();
    //ShaderWithColorVAO();
    ShaderWithShaderObject();
}
